...
func parse(t *testing.T, src string) *parser2.Module {
	t.Helper()
...
func TestModuleScope(t *testing.T) {
	...
	if sm.Root.Kind != symbols2.ScopeModule {
...
`)
	...
	if fn.Kind != symbols2.ScopeFunction {
	...
	a := fn.Symbols["a"]
	if a == nil || !a.Has(symbols2.FlagParam) {
		t.Error("a should be a param")
	}
	c := fn.Symbols["c"]
	if c == nil || !c.Has(symbols2.FlagBound) {
...
`)
	...
	if cls.Kind != symbols2.ScopeClass {
...
func TestImport(t *testing.T) {
	...
	if osB == nil || !osB.Has(symbols2.FlagImport) {
		t.Error("os should be import-bound")
	}
	argv := sm.Root.Symbols["argv"]
	if argv == nil || !argv.Has(symbols2.FlagImport) {
...
`)
	...
	if dest == nil || !dest.Has(symbols2.FlagBound) {
		t.Error("dest should be bound by match pattern")
	}
	action := sm.Root.Symbols["action"]
	if action == nil || !action.Has(symbols2.FlagBound) {
...
func TestLambdaScope(t *testing.T) {
	...
	if lam.Kind != symbols2.ScopeLambda {
...
func TestComprehensionScope(t *testing.T) {
	...
	if comp.Kind != symbols2.ScopeComprehension {
