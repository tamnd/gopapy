...
func checkF821(sm *symbols2.Module, mod *parser2.Module) []diag.Diagnostic {
	...
	var walk func(s *symbols2.Scope)
	walk = func(s *symbols2.Scope) {
		for name, sym := range s.Symbols {
			if !sym.Has(symbols2.FlagUsed) {
				continue
			}
			if sym.Has(symbols2.FlagBound) || sym.Has(symbols2.FlagGlobal) || sym.Has(symbols2.FlagNonlocal) {
				continue
			}
...
func hasModuleStarImport(mod *parser2.Module) bool {
